...
def get_train_transforms():
    return A.Compose(
        [
            A.Resize(config['img_size'], config['img_size']),  # 去掉 always_apply
            A.Normalize(),
            ToTensorV2(p=1.0)
        ])
...
model = Model()
model.load_state_dict(torch.load('vqvae.bin',map_location='cpu'))
...
def get_rec_image(filename, output_path, perturb_dim=None, epsilon=0.01):
    """
    filename: 输入图片路径（可以绝对路径）
    output_path: 输出图片完整路径（包含文件名）
    """
    # 读取图像并预处理
    ...
    with torch.no_grad():
        ...
        _, quantized = model.vq(model.vq_conv(x))
        if perturb_dim is not None and 0 <= perturb_dim < quantized.shape[1]:
            quantized = quantized.clone()
            quantized[:, perturb_dim, :, :] += epsilon
        predictions = model.decode(quantized)

    # 还原到 [0,255]
    predictions = predictions.squeeze(0).detach().cpu() * 255.0
    rec_image = predictions.permute(1, 2, 0).numpy()
