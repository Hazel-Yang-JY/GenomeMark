...
# 是否是增强图像
# -------------------------------
def is_augmented(s: str) -> bool:
    return "_aug_" in s or "_noise_" in s


# -------------------------------
# trigger 评估指标（conf & margin）
# -------------------------------
# def eval_trigger_metrics(model, loader, device, class_idx, conf_thresh=0.8, margin_thresh=0.5):
#     model.eval()
#     softmax = nn.Softmax(dim=1)
#     total, high_conf, high_margin = 0, 0, 0
#     with torch.no_grad():
...
#             margins = logits[:, class_idx] - torch.topk(logits, 2).values[:, 1]

#             high_conf += (confs >= conf_thresh).sum().item()
#             high_margin += (margins >= margin_thresh).sum().item()
#             total += imgs.size(0)

#     return 100.0 * high_conf / total, 100.0 * high_margin / total
...
# 主训练函数
# -------------------------------
def train(imagenet_root,
          save_path="resnet50_ft_center_margin.pth",
    ...
    val_dir = os.path.join(imagenet_root, "val", "main")
    ...
    trigger_full_set = datasets.ImageFolder(
        os.path.join(imagenet_root, "val", "trigger"),
        transform=transform
    )
    trigger_loader = DataLoader(trigger_full_set, batch_size=batch_size, shuffle=False)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch_idx, (imgs, labels) in pbar:
            ...
            for i in range(len(imgs)):
                ...
                if is_augmented(path) and labels[i].item() == target_idx:
                    # Center loss
...
if __name__ == '__main__':
    train(
    imagenet_root="imagenet_a",
    save_path="resnet50_anchor2.pth",
    target_class="beerbottle"
)